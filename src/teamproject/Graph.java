...
public class Graph<E>
{
    ...
    protected LinkedStack<Link> removedStack = new LinkedStack<>();
    class Link<E>
    {
        E start;
        E end;
        Double weight;
        Link(E start, E end, Double weight)
        {
            this.start = start;
            this.end = end;
            this.weight = weight;
    ...
    public boolean remove(E start, E end)
    {
        ...
        if( startVertex != null )
        {
            ...
            if (removedOK)
                removedStack.push(new Link(start, end, endPair.second));
        }
    ...
    {
        Link link = removedStack.pop();
        addEdge((E)link.start, (E)link.end, link.weight);
    }
    ...
    public void outputText(PrintWriter outFile){
        Iterator<Entry<E, Vertex<E>>> iter1;
        Iterator<Entry<E, Pair<Vertex<E>, Double>>> iter2;
        Entry<E, Pair<Vertex<E>, Double>> entry;
        Pair<Vertex<E>, Double> pair;
        
        iter1 = vertexSet.entrySet().iterator();
        while( iter1.hasNext() )
        {
            Vertex thisVertex = iter1.next().getValue();
            
            outFile.print( "Adj List for " + thisVertex.data + ": ");
            iter2 = thisVertex.adjList.entrySet().iterator();
            while( iter2.hasNext() )
            {
                entry = iter2.next();
                pair = entry.getValue();
                outFile.print( pair.first.data + "("
                        + String.format("%3.1f", pair.second)
                        + ") " );
            }
            outFile.println();
