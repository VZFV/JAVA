...
public class LList2<T> implements ListInterface<T>
{
   ...
   public boolean add(int newPosition, T newEntry)
	...
	{
		boolean found = false;
		Node2 currentNode = firstDummyNode.getNextNode(); // EX. 4.2

		while (!found && (currentNode != lastDummyNode)) // EX. 4.2
		{
			if (anEntry.equals(currentNode.getData()))
				found = true;
			else
			{
				//prevNode = currentNode; // EX. 4.2
		...
		Node2 nextNode = currentNode.getNextNode(); // EX. 4.2
		Node2 prevNode = currentNode.getPrevNode();	// EX. 4.2
